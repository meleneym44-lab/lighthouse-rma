    ...
    <header className="bg-lh-dark text-white sticky top-0 z-40 shadow-lg">
      <div className="max-w-6xl mx-auto px-4">
        <div className="flex justify-between items-center py-3">
          <div className="flex items-center gap-4 cursor-pointer" onClick={() => setPage(user ? 'dashboard' : 'home')}><Logo className="h-10" /></div>
          <div className="flex items-center gap-4">
            ...
            {user ? (
              <div className="flex items-center gap-3">
                ...
                <div className="w-10 h-10 rounded-full bg-lh-yellow text-lh-dark flex items-center justify-center font-bold text-lg">{profile?.full_name?.charAt(0) || 'U'}</div>
                <button onClick={onLogout} className="text-sm opacity-70 hover:opacity-100 ml-2">{t('logout')}</button>
              </div>
            ) : (
              <button onClick={() => setPage('login')} className="btn btn-sm bg-lh-yellow text-lh-dark hover:bg-yellow-400 font-semibold">{t('login')}</button>
            )}
          </div>
        </div>
        {user && (
          <nav className="flex gap-1 pb-2 overflow-x-auto">
            {nav.map(item => (
              <button key={item.id} onClick={() => setPage(item.id)} className={`nav-item flex items-center gap-2 ${page === item.id ? 'nav-active' : 'nav-inactive'}`}>
    ...
    <div>
      <section className="bg-gradient-to-br from-lh-dark via-lh-blue to-lh-dark text-white py-24 px-4">
        <div className="max-w-4xl mx-auto text-center">
          <div className="border-l-4 border-lh-yellow pl-6 inline-block mb-6"><h1 className="text-4xl md:text-5xl font-bold">{t('heroTitle')}</h1></div>
          <p className="text-xl opacity-90 mb-10">{t('heroSubtitle')}</p>
          <div className="flex gap-4 justify-center flex-wrap">
            <button onClick={() => setPage('login')} className="btn btn-lg bg-white text-lh-dark hover:bg-gray-100 font-semibold shadow-lg">{t('login')}</button>
      ...
      <section className="py-20 px-4 bg-white">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid md:grid-cols-4 gap-10">
            {[{ icon: '📝', num: 1, title: t('step1Title'), desc: t('step1Desc') },{ icon: '📨', num: 2, title: t('step2Title'), desc: t('step2Desc') },{ icon: '💰', num: 3, title: t('step3Title'), desc: t('step3Desc') },{ icon: '🚚', num: 4, title: t('step4Title'), desc: t('step4Desc') }].map(step => (
      ...
      <section className="py-20 px-4 bg-gray-50">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {[{ icon: '⚙️', title: t('calibration'), desc: t('calibrationDesc'), price: `${t('fromPrice')} 320€` },{ icon: '🔧', title: t('repair'), desc: t('repairDesc'), price: t('onQuote') },{ icon: '🔬', title: t('diagnostic'), desc: t('diagnosticDesc'), price: '100€' }].map((svc, i) => (
  ...
  return (
    <div className="min-h-[80vh] flex items-center justify-center px-4 py-12">
      <div className="card w-full max-w-md overflow-hidden">
        <div className="bg-lh-dark p-6 text-center"><Logo className="h-12 mx-auto mb-2" /></div>
        <div className="card-body">
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="form-group"><label className="label">{t('email')}</label><input type="email" value={email} onChange={e => setEmail(e.target.value)} className="input" required /></div>
            <div className="form-group"><label className="label">{t('password')}</label><input type="password" value={password} onChange={e => setPassword(e.target.value)} className="input" required /></div>
            {error && <p className="text-red-500 text-sm bg-red-50 p-3 rounded">{error}</p>}
            <button type="submit" disabled={loading} className="btn btn-primary w-full py-3 text-lg">{loading ? t('loading') : t('login')}</button>
          </form>
          <p className="text-center mt-6 text-gray-500">{t('noAccount')} <button onClick={() => setPage('register')} className="text-lh-blue font-semibold hover:underline">{t('register')}</button></p>
        </div>
      </div>
    </div>
  );
...
function RegisterPage({ t, lang, onRegister, setPage }) {
  ...
  const [error, setError] = useState('');
  const [loading, setLoading] = useState(false);
  const handleSubmit = async (e) => { e.preventDefault(); if (form.password !== form.confirmPassword) { setError(lang === 'fr' ? 'Les mots de passe ne correspondent pas' : 'Passwords do not match'); return; } setLoading(true); setError(''); const err = await onRegister(form); if (err) setError(err); setLoading(false); };
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8 space-y-8">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div><h1 className="text-2xl font-bold text-gray-800">{t('welcomeBack')}, {profile?.full_name}!</h1><p className="text-gray-500">{profile?.companies?.name}</p></div>
        <button onClick={() => setPage('new-request')} className="btn btn-primary">➕ {t('newRequest')}</button>
      </div>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {stats.map((s, i) => (<div key={i} className={`stat-card ${s.color}`}><div className="flex justify-between items-start"><div><div className="text-3xl font-bold text-gray-800">{s.value}</div><div className="text-sm text-gray-500 mt-1">{s.label}</div></div><div className="text-3xl">{s.icon}</div></div></div>))}
      </div>
      <div className="card">
        <div className="card-header flex justify-between items-center"><h2 className="card-title">{t('recentRequests')}</h2><button onClick={() => setPage('requests')} className="text-lh-blue text-sm font-medium hover:underline">{t('viewAll')} →</button></div>
        {requests.length === 0 ? (<div className="card-body text-center py-12"><div className="text-5xl mb-3">📋</div><p className="text-gray-500 mb-4">{t('noRequests')}</p><button onClick={() => setPage('new-request')} className="btn btn-primary">{t('newRequest')}</button></div>) : (
          <div className="overflow-x-auto"><table className="w-full"><thead><tr><th className="th">#</th><th className="th">{t('model')}</th><th className="th">{t('serviceType')}</th><th className="th">Status</th><th className="th">Date</th></tr></thead><tbody className="divide-y">
            {requests.slice(0, 5).map(r => { const st = STATUS_CONFIG[r.status] || STATUS_CONFIG.pending; return (<tr key={r.id} className="hover:bg-gray-50"><td className="td font-mono text-lh-blue font-semibold">{r.request_number || '-'}</td><td className="td">{r.model_name || '-'}</td><td className="td capitalize">{r.requested_service}</td><td className="td"><span className={`badge ${st.bg} ${st.text}`}>{st.icon} {t(r.status?.replace('_', ''))}</span></td><td className="td text-gray-500 text-sm">{r.created_at ? new Date(r.created_at).toLocaleDateString() : '-'}</td></tr>); })}
          </tbody></table></div>
        )}
      </div>
    </div>
...
function CustomerEquipment({ equipment, t, lang, profile, onRefresh, notify }) {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8 space-y-6">
      <div className="flex justify-between items-center"><h1 className="section-title">{t('myEquipment')}</h1><button onClick={() => setShowModal(true)} className="btn btn-primary">➕ {t('addEquipment')}</button></div>
      {equipment.length === 0 ? (<div className="card card-body text-center py-16"><div className="text-6xl mb-4">🔧</div><h2 className="text-xl font-semibold mb-2">{t('noEquipment')}</h2><p className="text-gray-500 mb-6">{t('addEquipmentFirst')}</p><button onClick={() => setShowModal(true)} className="btn btn-primary">{t('addEquipment')}</button></div>) : (
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
          {equipment.map(eq => (<div key={eq.id} className="card card-body hover:shadow-md transition-shadow"><div className="flex justify-between items-start mb-3"><div className="text-3xl">🔬</div><span className={`badge ${eq.equipment_type === 'biocollector' ? 'bg-yellow-100 text-yellow-700' : 'bg-blue-100 text-blue-700'}`}>{eq.equipment_type === 'biocollector' ? t('biocollector') : t('particleCounter')}</span></div><h3 className="font-bold text-lg text-lh-dark">{eq.model_name || 'Unknown'}</h3><p className="font-mono text-sm text-gray-500">{eq.serial_number}</p>{eq.customer_location && <p className="text-sm text-gray-400 mt-2">📍 {eq.customer_location}</p>}</div>))}
        </div>
      )}
      {showModal && (
        <div className="modal-overlay" onClick={() => setShowModal(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div className="form-group"><label className="label">{t('type')}</label><select value={form.equipment_type} onChange={e => setForm({...form, equipment_type: e.target.value})} className="input"><option value="particle_counter">{t('particleCounter')}</option><option value="biocollector">{t('biocollector')}</option><option value="other">{t('other')}</option></select></div>
              ...
              <div className="flex gap-3 pt-2"><button type="button" onClick={() => setShowModal(false)} className="btn btn-secondary flex-1">{t('cancel')}</button><button type="submit" disabled={saving} className="btn btn-primary flex-1">{saving ? t('saving') : t('save')}</button></div>
            </form>
          </div>
        </div>
      )}
...
function CustomerRequests({ requests, t, lang, setPage }) {
  return (
    <div className="max-w-6xl mx-auto px-4 py-8 space-y-6">
      <div className="flex justify-between items-center"><h1 className="section-title">{t('myRequests')}</h1><button onClick={() => setPage('new-request')} className="btn btn-primary">➕ {t('newRequest')}</button></div>
      {requests.length === 0 ? (<div className="card card-body text-center py-16"><div className="text-6xl mb-4">📋</div><h2 className="text-xl font-semibold mb-2">{t('noRequests')}</h2><button onClick={() => setPage('new-request')} className="btn btn-primary mt-4">{t('newRequest')}</button></div>) : (
        <div className="card overflow-hidden"><table className="w-full"><thead><tr><th className="th">#</th><th className="th">{t('serialNumber')}</th><th className="th">{t('model')}</th><th className="th">{t('serviceType')}</th><th className="th">Status</th><th className="th">Date</th></tr></thead><tbody className="divide-y">
          {requests.map(r => { const st = STATUS_CONFIG[r.status] || STATUS_CONFIG.pending; return (<tr key={r.id} className="hover:bg-gray-50"><td className="td font-mono text-lh-blue font-semibold">{r.request_number || '-'}</td><td className="td font-mono text-sm">{r.serial_number}</td><td className="td">{r.model_name || '-'}</td><td className="td capitalize">{r.requested_service}</td><td className="td"><span className={`badge ${st.bg} ${st.text}`}>{st.icon} {t(r.status?.replace('_', ''))}</span></td><td className="td text-gray-500 text-sm">{r.created_at ? new Date(r.created_at).toLocaleDateString() : '-'}</td></tr>); })}
        </tbody></table></div>
      )}
    </div>
  );
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-8">
      ...
      <form onSubmit={handleSubmit} className="card card-body space-y-6">
        <div className="form-group"><label className="label">{t('selectEquipment')} *</label><select value={form.equipment_id} onChange={e => setForm({...form, equipment_id: e.target.value})} className="input" required><option value="">-- {lang === 'fr' ? 'Sélectionner' : 'Select'} --</option>{equipment.map(eq => <option key={eq.id} value={eq.id}>{eq.model_name || 'Unknown'} - {eq.serial_number}</option>)}</select></div>
        <div className="form-group"><label className="label">{t('serviceType')} *</label><div className="grid grid-cols-2 gap-3">{[{ value: 'calibration', icon: '⚙️', label: t('calibration') },{ value: 'repair', icon: '🔧', label: t('repair') },{ value: 'calibration_repair', icon: '⚙️🔧', label: t('calibrationRepair') },{ value: 'diagnostic', icon: '🔬', label: t('diagnostic') }].map(opt => (<label key={opt.value} className={`flex items-center gap-3 p-4 rounded-lg border-2 cursor-pointer transition-all ${form.requested_service === opt.value ? 'border-lh-blue bg-lh-light' : 'border-gray-200 hover:border-gray-300'}`}><input type="radio" name="service" value={opt.value} checked={form.requested_service === opt.value} onChange={e => setForm({...form, requested_service: e.target.value})} className="hidden" /><span className="text-xl">{opt.icon}</span><span className="font-medium">{opt.label}</span></label>))}</div></div>
        <div className="form-group"><label className="label">{t('problemDescription')}</label><textarea value={form.problem_description} onChange={e => setForm({...form, problem_description: e.target.value})} className="input min-h-[120px]" placeholder={lang === 'fr' ? 'Décrivez le problème ou les symptômes...' : 'Describe the problem or symptoms...'} /></div>
        <div className="form-group"><label className="label">{t('urgency')}</label><div className="flex gap-6">{['normal', 'urgent', 'critical'].map(u => (<label key={u} className="flex items-center gap-2 cursor-pointer"><input type="radio" name="urgency" value={u} checked={form.urgency === u} onChange={e => setForm({...form, urgency: e.target.value})} className="w-4 h-4 text-lh-blue" /><span className={`font-medium ${u === 'critical' ? 'text-red-600' : u === 'urgent' ? 'text-orange-600' : ''}`}>{t(u)}</span></label>))}</div></div>
        {addresses.length > 0 && (<div className="form-group"><label className="label">{t('shippingAddresses')}</label><select value={form.shipping_address_id} onChange={e => setForm({...form, shipping_address_id: e.target.value})} className="input">{addresses.map(a => <option key={a.id} value={a.id}>{a.label} - {a.address_line1}, {a.city}</option>)}</select></div>)}
        <div className="flex gap-3 pt-4"><button type="button" onClick={() => setPage('dashboard')} className="btn btn-secondary flex-1">{t('cancel')}</button><button type="submit" disabled={submitting || !form.equipment_id} className="btn btn-primary flex-1">{submitting ? t('saving') : t('submit')}</button></div>
      </form>
    </div>
  );
}
</th><th className="th">{t('model')}</th><th className="th">{t('serviceType')}</th><th className="th">Status</th></tr></thead><tbody className="divide-y">
          {requests.slice(0, 10).map(r => { const st = STATUS_CONFIG[r.status] || STATUS_CONFIG.pending; return (<tr key={r.id} className="hover:bg-gray-50"><td className="td font-mono text-lh-blue font-semibold">{r.request_number || '-'}</td><td className="td font-medium">{r.companies?.name || '-'}</td><td className="td">{r.model_name || '-'}</td><td className="td capitalize">{r.requested_service}</td><td className="td"><span className={`badge ${st.bg} ${st.text}`}>{st.icon} {r.status}</span></td></tr>); })}
        </tbody></table></div>
      </div>
    </div>
  );
...
function AdminRequests({ requests, t, lang, onRefresh, notify }) {
  ...
  const updateStatus = async (id, newStatus) => { const { error } = await supabase.from('service_requests').update({ status: newStatus }).eq('id', id); if (error) notify(error.message, 'error'); else { notify(t('saved')); onRefresh(); } };
  const sendQuote = async () => { setSaving(true); const { error } = await supabase.from('service_requests').update({ status: 'quoted', quote_calibration: quoteForm.calibration, quote_parts: quoteForm.parts, quote_labor_hours: quoteForm.labor_hours, quote_labor_rate: quoteForm.labor_rate, quote_shipping: quoteForm.shipping, quote_notes: quoteForm.notes, quote_subtotal: subtotal, quote_tax: tax, quote_total: total, quoted_at: new Date().toISOString() }).eq('id', selectedRequest.id); setSaving(false); if (error) notify(error.message, 'error'); else { notify(t('saved')); setSelectedRequest(null); onRefresh(); } };
  return (
    <div className="max-w-6xl mx-auto px-4 py-8 space-y-6">
      <h1 className="section-title">{t('requests')}</h1>
      <div className="flex gap-2 flex-wrap">{filters.map(f => (<button key={f} onClick={() => setFilter(f)} className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${filter === f ? 'bg-lh-blue text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>{t(f)} ({f === 'all' ? requests.length : requests.filter(r => r.status === f).length})</button>))}</div>
      <div className="card overflow-hidden"><table className="w-full"><thead><tr><th className="th">#</th><th className="th">{t('company')}</th><th className="th">{t('serialNumber')}</th><th className="th">{t('model')}</th><th className="th">{t('serviceType')}</th><th className="th">{t('urgency')}</th><th className="th">Status</th><th className="th">Actions</th></tr></thead><tbody className="divide-y">
        {filtered.map(r => { const st = STATUS_CONFIG[r.status] || STATUS_CONFIG.pending; return (
      ...
      {selectedRequest && (
        <div className="modal-overlay" onClick={() => setSelectedRequest(null)}>
          <div className="modal max-w-2xl" onClick={e => e.stopPropagation()}>
            <div className="bg-lh-dark text-white p-4"><h2 className="font-semibold text-lg">{t('createQuote')}</h2><p className="text-sm opacity-70">{selectedRequest.request_number} - {selectedRequest.companies?.name}</p></div>
            <div className="p-6 space-y-6">
              <div className="bg-gray-50 rounded-lg p-4"><h3 className="font-semibold mb-3">{lang === 'fr' ? 'Détails de la demande' : 'Request Details'}</h3><div className="grid grid-cols-2 gap-3 text-sm"><div><span className="text-gray-500">{t('serialNumber')}:</span> <strong>{selectedRequest.serial_number}</strong></div><div><span className="text-gray-500">{t('model')}:</span> <strong>{selectedRequest.model_name}</strong></div><div><span className="text-gray-500">{t('serviceType')}:</span> <strong className="capitalize">{selectedRequest.requested_service}</strong></div><div><span className="text-gray-500">{t('urgency')}:</span> <strong className="capitalize">{selectedRequest.urgency}</strong></div></div>{selectedRequest.problem_description && (<div className="mt-3 text-sm"><span className="text-gray-500">{t('problemDescription')}:</span><p className="mt-1">{selectedRequest.problem_description}</p></div>)}</div>
              <div className="grid grid-cols-2 gap-4"><div className="form-group"><label className="label">{t('calibration')} (€)</label><input type="number" value={quoteForm.calibration} onChange={e => setQuoteForm({...quoteForm, calibration: Number(e.target.value)})} className="input" /></div><div className="form-group"><label className="label">{t('parts')} (€)</label><input type="number" value={quoteForm.parts} onChange={e => setQuoteForm({...quoteForm, parts: Number(e.target.value)})} className="input" /></div><div className="form-group"><label className="label">{t('labor')} ({t('hours')})</label><input type="number" step="0.5" value={quoteForm.labor_hours} onChange={e => setQuoteForm({...quoteForm, labor_hours: Number(e.target.value)})} className="input" /></div><div className="form-group"><label className="label">{t('rate')} (€/h)</label><input type="number" value={quoteForm.labor_rate} onChange={e => setQuoteForm({...quoteForm, labor_rate: Number(e.target.value)})} className="input" /></div><div className="form-group"><label className="label">{t('shipping')} (€)</label><input type="number" value={quoteForm.shipping} onChange={e => setQuoteForm({...quoteForm, shipping: Number(e.target.value)})} className="input" /></div></div>
              ...
              <div className="bg-gray-50 rounded-lg p-4 space-y-2"><div className="flex justify-between"><span className="text-gray-600">{t('totalHT')}:</span><span className="font-semibold">{subtotal.toFixed(2)} €</span></div><div className="flex justify-between"><span className="text-gray-600">{t('tva')}:</span><span className="font-semibold">{tax.toFixed(2)} €</span></div><div className="flex justify-between text-lg border-t pt-2"><span className="font-semibold">{t('totalTTC')}:</span><span className="font-bold text-lh-dark">{total.toFixed(2)} €</span></div></div>
            </div>
...
function AdminClients({ companies, equipment, t, lang }) {
  return (
    <div className="max-w-6xl mx-auto px-4 py-8 space-y-6">
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">{companies.map(c => { const devCount = equipment.filter(e => e.company_id === c.id).length; const contact = c.company_contacts?.[0]; return (<div key={c.id} className="card card-body border-t-4 border-lh-blue hover:shadow-md transition-shadow"><div className="flex justify-between items-start mb-3"><h3 className="font-bold text-lg text-lh-dark">{c.name}</h3><span className="badge bg-blue-100 text-blue-700">{devCount} {lang === 'fr' ? 'appareils' : 'devices'}</span></div><div className="space-y-1 text-sm text-gray-600">{contact && <p>👤 {contact.full_name}</p>}<p>📧 {c.email || '-'}</p><p>📞 {c.phone || '-'}</p><p>📍 {c.billing_postal_code} {c.billing_city}</p></div></div>); })}</div>
    </div>
  );
}
...
function AdminEquipment({ equipment, t, lang }) {
  return (
    <div className="max-w-6xl mx-auto px-4 py-8 space-y-6">
      ...
      <div className="card overflow-hidden"><table className="w-full"><thead><tr><th className="th">{t('serialNumber')}</th><th className="th">{t('model')}</th><th className="th">{t('type')}</th><th className="th">{t('company')}</th><th className="th">{t('location')}</th></tr></thead><tbody className="divide-y">
        {equipment.map(eq => (<tr key={eq.id} className="hover:bg-gray-50"><td className="td font-mono font-semibold">{eq.serial_number}</td><td className="td">{eq.model_name || '-'}</td><td className="td"><span className={`badge ${eq.equipment_type === 'biocollector' ? 'bg-yellow-100 text-yellow-700' : 'bg-blue-100 text-blue-700'}`}>{eq.equipment_type === 'biocollector' ? 'Bio' : 'Counter'}</span></td><td className="td">{eq.companies?.name || '-'}</td><td className="td text-gray-500">{eq.customer_location || '-'}</td></tr>))}
      </tbody></table></div>
    </div>
  );
}
